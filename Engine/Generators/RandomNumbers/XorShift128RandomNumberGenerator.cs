...
namespace Aximo.Generators.RandomNumbers
{
    ...
    ///
    /// </summary>
    public class XorShift128RandomNumberGenerator : RandomNumberGeneratorBase
    {
        ...
        /// </summary>
        public XorShift128RandomNumberGenerator()
        {
            // Initialise using the system tick count.
            Reinitialise((uint)Environment.TickCount);
        }

        public XorShift128RandomNumberGenerator(int seed)
        {
            unchecked
            {
                Reinitialise((uint)seed);
            }
        }
        ...
        /// System.Random
        /// </summary>
        public XorShift128RandomNumberGenerator(uint seed)
        {
            Reinitialise(seed);
        }
        ...
        public override int Next(int upperBound)
        {
            unchecked
            {
                ...
                return (int)((IntToDoubleMultiplier * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8))))) * upperBound);
            }
        }
        ...
        public override int Next(int lowerBound, int upperBound)
        {
            unchecked
            {
                ...
                if (range < 0)
                    return lowerBound + (int)((UIntToDoubleMultiplier * (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8)))) * (upperBound - (long)lowerBound));

                // 31 bits of precision will suffice if range<=int.MaxValue. This allows us to cast to an int and gain
                // a little more performance.
                return lowerBound + (int)((IntToDoubleMultiplier * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8))))) * range);
            }
        }
        ...
        public override double NextDouble()
        {
            unchecked
            {
                ...
                // System.Random.
                return IntToDoubleMultiplier * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8))));
            }
        }
